...
func _input(event : InputEvent):
	if (event is InputEventKey):
		if (event.physical_keycode == 96): # Reverse-nice.  Also ~ key.
			if (event.pressed):
				toggle_console()
			get_viewport().set_input_as_handled()
		elif (event.physical_keycode == KEY_ESCAPE && control.visible): # Disable console on ESC
			if (event.pressed):
				toggle_console()
				get_viewport().set_input_as_handled()
	if (control.visible):
...
func add_command(command_name : String, function : Callable, param_count : int = 0):
